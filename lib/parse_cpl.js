...
var Stream = require('stream'),
	...
	parseDCName = require('./dcp_name_parser').parse;
 ...
 *
 * Options: parseDCName - if set to true, then the DCP
 *   naming parser will be used to populate the CPL metadata 
 *
 * Example:
 *
	...
	saxStream.on('text', function (t) {
		...
		if (level === 'root')
			switch (tag) {
				...
				case 'ANNOTATIONTEXT':
					cpl.text = t;
					if (options.parseDCName)
						_.extend(cpl, parseDCName(cpl.text));
					break;
